...
class PerpetualMarketMakingStrategy(StrategyPyBase):
    ...
    def get_price(self) -> float:
        ...
        # Final fallback: ask the connector directly (uses synthetic price when book is empty).
        if price.is_nan():
            market = self._market_info.market
            trading_pair = self._market_info.trading_pair
            bid = market.get_price(trading_pair, False)
            ask = market.get_price(trading_pair, True)
            if not bid.is_nan() and not ask.is_nan():
                price = (bid + ask) / 2
            elif not bid.is_nan():
                price = bid
            elif not ask.is_nan():
    ...
    def active_orders_df(self) -> pd.DataFrame:
        ...
        for idx in range(0, len(active_orders)):
            ...
            amount_orig = (
                ""
                if level is None
                else self._order_amount + ((level - 1) * self._order_level_amount)
            )
            data.append(
                [
                    level,
    ...
    def market_status_data_frame(self) -> pd.DataFrame:
        ...
        if (
            self._price_type is PriceType.LastOwnTrade
            and self._last_own_trade_price.is_nan()
        ):
        ...
        if type(self._asset_price_delegate) is OrderBookAssetPriceDelegate:
            market_books.append(
                (
                    self._asset_price_delegate.market,
        ...
        for market, trading_pair in market_books:
            ...
            if (
                market == self._market_info.market
                and self._asset_price_delegate is None
            ):
                ref_price = self.get_price()
            elif (
                market == self._asset_price_delegate.market
        ...
        return pd.DataFrame(data=markets_data, columns=markets_columns).replace(
            np.nan, "", regex=True
        )
    ...
    def format_status(self) -> str:
        ...
        df_lines = assets_df.to_string(
            index=False,
            header=False,
            formatters={0: ("{:<" + str(first_col_length) + "}").format},
        ).split("\n")
    ...
    def tick(self, timestamp: float):
        ...
        session_positions = [
            s
            for s in self.active_positions.values()
    ...
    def profit_taking_proposal(
        self, mode: PositionMode, active_positions: List
    ) -> Proposal:
            ...
            if len(active_positions) > 1:
                ...
                for order in unwanted_exit_orders:
                    if (active_positions[0].amount < 0 and order.is_buy) or (
                        ...
                        self.logger().info(
                            f"Initiated cancelation of {'buy' if order.is_buy else 'sell'} order "
        ...
        for position in active_positions:
            if (ask_price > position.entry_price and position.amount > 0) or (
                ...
                profit_spread = (
                    self._long_profit_taking_spread
                    if position.amount > 0
                    else self._short_profit_taking_spread
                )
                take_profit_price = (
                ...
                price = market.quantize_order_price(
                    self.trading_pair, take_profit_price
                )
                ...
                old_exit_orders = [
                    ...
                    if (
                        ...
                        and (
                            (position.amount < 0 and o.is_buy)
    ...
    def stop_loss_proposal(
        self, mode: PositionMode, active_positions: List[Position]
    ) -> Proposal:
        market: DerivativeBase = self._market_info.market
        ...
        if top_ask is None or top_bid is None or top_ask.is_nan() or top_bid.is_nan():
            return Proposal(buys, sells)
        ...
        for position in active_positions:
            ...
            existent_stop_loss_orders = [
                ...
                and (
                    (position.amount > 0 and not order.is_buy)
            ...
            if not existent_stop_loss_orders or (
                ...
                for order in existent_stop_loss_orders:
                    ...
                    self.cancel_order(self._market_info, order.client_order_id)
                    self.logger().info(
                        f"Canceling the limit order {order.client_order_id} to renew stop loss."
                    )
        ...
        if order_override is not None and len(order_override) > 0:
            for key, value in order_override.items():
                if str(value[0]) in ["buy", "sell"]:
                    if str(value[0]) == "buy":
                        price = self.get_price() * (
                            Decimal("1") - Decimal(str(value[1])) / Decimal("100")
                        )
            ...
            for level in range(0, self._buy_levels):
                price = self.get_price() * (
                    Decimal("1") - self._bid_spread - (level * self._order_level_spread)
                )
                price = market.quantize_order_price(self.trading_pair, price)
    ...
    def apply_budget_constraint(self, proposal: Proposal):
        ...
        adjusted_candidates = checker.adjust_candidates(
            order_candidates, all_or_none=True
        )
    ...
    def apply_order_optimization(self, proposal: Proposal):
        ...
        if len(proposal.buys) == 1:
            ...
            top_bid_price = self._market_info.get_price_for_volume(
                False, self._bid_order_optimization_depth + own_buy_size
            ).result_price
        ...
        if len(proposal.sells) == 1:
            ...
            top_ask_price = self._market_info.get_price_for_volume(
                True, self._ask_order_optimization_depth + own_sell_size
            ).result_price
            price_quantum = market.get_order_price_quantum(
    ...
    def did_change_position_mode_succeed(
    ...
    def cancel_active_orders(self, proposal: Proposal):
        ...
        if proposal is not None and self._order_refresh_tolerance_pct >= 0:
            ...
            active_sell_prices = [
                Decimal(str(o.price)) for o in self.active_orders if not o.is_buy
            ]
            ...
            ) and self.is_within_tolerance(active_sell_prices, proposal_sells):
    ...
    def cancel_orders_below_min_spread(self):
        ...
        for order in self.active_orders:
            ...
            if (negation * (order.price - price) / price) < self._minimum_spread:
                ...
                self.cancel_order(self._market_info, order.client_order_id)
                self.logger().info(
                    f"Canceling order {order.client_order_id} below min spread."
                )
    ...
    def execute_orders_proposal(
        self, proposal: Proposal, position_action: PositionAction
    ):
        ...
        if len(proposal.buys) > 0:
            if position_action == PositionAction.CLOSE:
                if self.current_timestamp < self._next_buy_exit_order_timestamp:
                    ...
                    self._next_buy_exit_order_timestamp = (
                        self.current_timestamp + self.filled_order_delay
                    )
        ...
        if len(proposal.sells) > 0:
            if position_action == PositionAction.CLOSE:
                if self.current_timestamp < self._next_sell_exit_order_timestamp:
                    ...
                    self._next_sell_exit_order_timestamp = (
                        self.current_timestamp + self.filled_order_delay
                    )
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                price_quote_str = [
